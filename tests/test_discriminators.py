...
def test_kde_discriminator():
    ...
    disc = dem.KdeDiscriminator(D=2, bw_init=1.0, trainable=False)
    disc.set_data_numpy(x0, x1)
    y_pred, y_prob = disc.classify(x)
    acc = dem.accuracy(y_pred=y_pred, y_true=labels)
    assert acc >= 0.0, "accuracy should be non-negative"
    dem.plot_disc_2d(disc, x, labels, save_name="test_out/kde_2d.png")
...
def test_kde_discriminator_training():
    ...
    out, _ = dem.train_occ(disc, x0, plot_freq=1, n_epochs=30, outdir="test_out/kde")
...
def test_nn_discriminator_training():
    ...
    disc = dem.NeuralDiscriminator(D=2)
    out, _ = dem.train_occ(disc, x1, plot_freq=1, n_epochs=30, outdir="test_out/nn")
    ea = out.read_logged_events()
    df = out.read_logged_scalar(name="valid_accuracy")
    assert df.shape[0] == 30, "logs must have a row for each epoch!"
